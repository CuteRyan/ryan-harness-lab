...
- [2026-04-28] 진행 중 | /feedback 스킬 B 방식 1·2단계 완료 + 3단계 (정보 제공형 훅) 별도 세션 분리 | 다음: 3단계 훅 디자인 (sycophancy 키워드 사전 + 환각 자동 검출, settings.json 등록, 차단형 X 표시형 O) | 미결: 없음 (1·2단계는 모두 commit·push 완료)
...
| 날짜 | Day | 요약 | 파일 |
|------|-----|------|------|
| 2026-04-28 | 14 | **/feedback B방식 (프롬프트 파일화) + 메인 Claude 5게이트** — AM: A2 검증 종결(영어 전환 38%→40% 미미, Claude Sub G2' INVALID 회귀로 롤백) + NativeCommandError fix(EAP 격리). PM: B방식 1단계(`prompts/review.md` SSOT 신설, `prepare-isolation.ps1`이 격리 디렉토리 복사, orchestrate.ps1 짧은 메타 지시) → H 실측 **3/3 VALID** (이전 항상 1~2/3) + 비판 형식 준수 3/3 + Codex 마크다운 볼드 회귀 자동 해소. 2단계(SKILL.md Step 3 5게이트: 라인실측·반박최소·근거강도·통계표·자기비판 강제) → 종합 보고 작성 시 게이트 1·5 효과 즉시 확인 (1차 보고에서 빠뜨렸던 Gemini 라인 정밀도·시스템 사각지대 자동 노출) | [상세](2026-04-28.md) |
