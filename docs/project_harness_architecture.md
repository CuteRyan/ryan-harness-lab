...
> ⚠️ **ARCHIVED (2026-07-06) — 자동 로드 해제됨.** 하네스 다이어트 감사 결과, 본 문서가 설계한 "문서 하네스" 서브시스템(`.harness.yml` opt-in + 역색인 + doc-template/code-doc-sync 훅)의 **실사용 소비자가 전 프로젝트에 0건**으로 확인됨(PAA·election_simulator·candidate·harness 어디에도 `.harness.yml`/`.harness-index.json` 없음). 관련 훅 4종은 미배선(settings.json 참조 0) 상태로 삭제됨. 아래 "Phase 0~4 완료" 서술은 **설계·구현 시점(2026-04)의 스냅샷**이며 현재 라이브 상태가 아님. 기록 보존용으로만 유지. 되살리려면 `/project-structure init`으로 opt-in 파일을 실제 배치해야 함.
>
> 작성일: 2026-04-12 | 마지막 검증: 2026-04-13 | 아카이브: 2026-07-06
> 출처 신뢰도: 직접 조사 + 업계 표준 참고
> 상태: **[ARCHIVED] 설계 완료했으나 실배선/실사용 0 — 유령 서브시스템**
