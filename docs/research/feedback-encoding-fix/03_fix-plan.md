...
## L4 — Codex 내부 spawn 회복 단계 skip (2026-04-25 적용)

### 배경 (Day 10 Part 3 V-1 내부 관찰)

Day 10 Part 3 V-1 (한글 다수 파일 1회 실행) 동안 Codex 가 **자체 PowerShell 서브프로세스** 로 격리 복사본을 읽을 때 다음 시퀀스가 stdout 에 관찰됨:

1. `Get-Content <copied>.md` (PS 5.1 기본 CP949) → 파일 내용 mojibake 로 stdout 출력
2. Codex 가 mojibake 감지 → "파일은 UTF-8인데 기본 PS 인코딩으로 읽혔다" 자체 추론
3. `Get-Content <copied>.md -Encoding UTF8` 재시도 → 정상 복구

결과물은 깨끗했으나, 회복 1단계가 매번 발생 + 첫 stdout 이 mojibake 라 진단성 저하. 이 영역은 우리가 Codex 내부 호출을 직접 고칠 수 없음 (L4 = 우리 스킬 외부 레이어).

근거: `docs/history/2026-04-23.md` §V-1 내부 관찰.

### 우회책 — 격리 복사본에 BOM 삽입

**`prepare-isolation.ps1`** 가 격리 디렉토리로 복사할 때 **UTF-8 BOM 을 강제 삽입**. BOM `EF BB BF` 가 있으면 `Get-Content` 기본 모드에서도 .NET StreamReader 가 BOM detection 으로 UTF-8 인코딩을 즉시 인식 → mojibake 단계 자체가 사라짐.

### 적용 패치 요약

```powershell
# prepare-isolation.ps1 — Copy-Item 치환
New-Item -ItemType Directory -Path $isolated | Out-Null

# (구) Copy-Item -LiteralPath $SourceFile -Destination $isolated
# (신) BOM 강제 삽입 복사
...
- **회복 1단계 절약**: Codex spawn PS 의 CP949 → UTF8 재시도 cycle 제거
- **진단성 향상**: 첫 stdout 부터 정상 한글 → 재발 시 L1/L2/L3/L4 어느 레이어인지 즉시 판별
- **D1 결정과 호환**: D1(=출력 md BOM 유지)은 출력측 정책이고, L4 패치는 입력측(격리 복사본). 둘 다 BOM 유지 방향으로 일관

### 한계

- Codex 가 BOM 없는 다른 파일을 추가로 읽는 경로(예: 격리 디렉토리 외부 참조)에는 효과 없음. 격리 디렉토리 외부 접근은 본 스킬 설계상 차단 대상이라 실제 영향 없음
- 입력 파일 자체가 비-UTF-8(예: CP949) 인 경우 `Get-Content -Encoding UTF8` 단계에서 mojibake 가 격리본에 그대로 박힘. 본 스킬 입력은 프로젝트 md 만 대상이므로 UTF-8 가정 유효

### 검증

- ParseFile 통과 (PowerShell 문법 정상)
- 한글 다수 md (`2026-04-22.md`) 격리 복사 후 첫 3바이트 = `EF BB BF` 확인
- 스크립트 자체 BOM 유지 (`EF BB BF`)
- 운영 동기화 후 SHA256 MATCH
...
### 배경
- V-1 단발 호출은 OK / V-3 (3회 연속) 에서만 Codex/Gemini 가 retry 시 stateful 누적 실패.
- Codex retry 사유: `Reading additional input from stdin...` — 이전 Start-Job 자식의 stdin 핸들이 살아 있어 동기 retry 의 `$null |` 파이프가 곧장 EOF 처리되지 않음.
- Gemini retry 사유: `[ERROR] [IDEClient] Failed to connect to IDE companion extension` — IDE companion 클라이언트 핸들이 정리되기 전에 재호출되어 새 연결 거부.
- 실패 마커: `docs/feedback/2026-04-23_codex_2026-04-22_20260423-173731.md`.

### 적용 패치 (issue-cleanup-day12 / Task #1)

| 항목 | 파일 | 변경 |
|---|---|---|
| A1-1 | `orchestrate.ps1` | `[int]$WaitSec = 10` 매개변수 추가. Step 7 retry 직전, 최소 1개라도 실패가 있으면 `Start-Sleep -Seconds $WaitSec`. 이전엔 sleep 자체가 없어 자식 핸들 정리 시간이 0 이었음. |
| A1-2 | `run-codex.ps1` / `run-gemini.ps1` | 자체 retry 루프 — 지수 백오프 1→2→4s, 최대 3회. 단발 호출 시점의 일시적 stateful 잔재(Codex stdin / Gemini IDE client)에 대해 자체 회복. |
| A1-3 | `orchestrate.ps1` | `[switch]$Sequential` 추가 — Start-Job 우회, 3 CLI 직렬 호출. 병렬 runspace 자체가 누적 원인일 때의 안전 모드. Step 7 retry 로직과 호환 (동일 `$jobResults` 해시 사용). |

### V-3 가설
- WaitSec 10s 는 V-3 1차 실패에서 관측된 핸들 정리 지연(2~5s) 의 2배 마진. 3회 연속 실행 시 retry 분기에 도달하더라도 stdin/IDE companion 핸들이 안전하게 정리됨.
- 자체 retry 가 더해져, Step 7 최종 retry 까지 가는 빈도 자체가 줄어들 것으로 기대 (1차 단발이 자체 백오프로 자연 회복).
- V-3 가 여전히 실패하면 `-Sequential` 모드로 우회 가능 — 병렬 runspace 누적이 진짜 뿌리라는 증거가 됨. 그 경우 Step C(또는 별도 D) 후속 연구.

### 운영 동기화
- 스테이징: `C:\Users\rlgns\OneDrive\문서\Harness-engineering\skills\feedback\scripts\{orchestrate,run-codex,run-gemini}.ps1`
- 운영: `~/.claude/skills/feedback/scripts/{orchestrate,run-codex,run-gemini}.ps1`
- SHA256 MATCH 3 쌍 확인 (2026-04-25, Task #1 완료 시).
- ParseFile 통과 3 파일 (PowerShell 5.1 문법).

### 미반영 / 후속
- A2(`prepare-isolation.ps1` BOM 회복) — 별도 teammate(Task #2) 처리.
- 실측 V-3 3회 연속 호출 검증은 메인 세션이 별도 수행 (시간 비용으로 본 작업에서는 제외).
