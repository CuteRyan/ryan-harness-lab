...
5. **외부 리서치 + 출처 인용 의무** (#014 PASS, 2026-05-04 turn 9): 추천 시 자기 지식 (training data) 만으로 단언 금지.
   WebSearch / WebFetch / 외부 CLI 中 1순위 → 보조 순서로 외부 자료 검색 후 출처 인용 의무.
   추천마다 **4 요소** 강제: ① 결론 (1~2줄) ② 출처 (URL + 발행일 + 직접 인용 1~2줄, paraphrase 금지) ③ 추측 표현 금지 (`아마`·`보통`·`일반적으로` 등 사용 금지) ④ 자기비판 1줄 (이 추천의 약점·반박 가능성).
   글로벌 `~/.claude/rules/research-mandatory.md` superset + PM 한정 강도 추가.
   면제 예외: 코드/파일/git history/내부 메모리·환경변수는 외부 리서치 무관 (Read·Grep·Glob·git 으로 충분).
...
| **PM 외부 리서치 + 출처 인용 강제** | PM agent system prompt 에 외부 리서치 의무 (WebSearch/WebFetch/외부 CLI 1순위 + 보조) + 출처 형식 (URL + 발행일 + 직접 인용 1~2줄) + 4 요소 강제 (결론·출처·추측 금지·자기비판) 박음. 글로벌 `~/.claude/rules/research-mandatory.md` superset + PM 한정 강도 추가 (PM 은 추천 자체가 직무 → 매 추천마다 출처 의무). 본 가드레일은 §2.3 "핵심 행동 규칙 5번" 으로 명세, PM agent (`~/.claude/agents/pm.md` **정식 PM, 2026-05-04 turn 11 #009-A PASS 신설** + 11 워커 직책별 agent 동시 신설) frontmatter + system prompt 에 박음. ~~pm-test.md 임시~~ → archive (`.backups/agents/pm-test.md.archived_20260504-009A.md`). | 글로벌 `rules/research-mandatory.md` + #014 PASS (2026-05-04 turn 9) + #009-A PASS (turn 11) |
