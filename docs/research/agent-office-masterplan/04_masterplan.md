...
현재 인프라의 75% 이상이 이미 설계됨 — /feedback(⑤검수), v2 스펙(②회의실+④파이프라인). 남은 핵심 과제는 **3층 PM 구체화**와 **① 인턴 패턴 정의**다.
...
**리뷰 사이클 cap**: aws-samples에서 검증된 수치 — 리뷰 실패 3회 초과 시 PM에 에스컬레이션.
...
> Anthropic 블로그: "multi-agent는 single-agent 대비 ~15× 토큰."
...
| 외부 검증 필요 (③ 외부 CLI) | **선택** | /feedback 단발 특성 존중 |
...
```
PM 정의 frontmatter에만 `model: opus` 명시. 이중 보장으로 issue#32732 회피.
...
> **issue#32732 경고**: Opus main session이 Agent 호출 시 model 파라미터를 자동 추가 → frontmatter를 덮어쓸 수 있음. `CLAUDE_CODE_SUBAGENT_MODEL=sonnet` + PM frontmatter `model: opus` 이중 보장으로 대부분 해소. 실험 후 fallback 확인 필요.

### 8.3 비용 효과

- Sonnet ≈ Opus의 1/5 비용
- 워커 80% 비용 절감 (Anthropic 블로그 직접 근거)
- "+90.2% 성능 향상" = Opus(lead) + Sonnet(worker) 조합 (Anthropic 블로그 실험 결과)
...
| 고아 팀 청소 | `~/.claude/teams/` 글로벌 cleanup (validate-team.ps1) | v2 스펙 |
| 리뷰 사이클 cap | 3회 초과 시 PM 에스컬레이션 | aws-samples |
| spawn 범위 제한 | spawn prompt에 "이 경로 외 탐색 금지" 명시 | issue#35513 |

### 9.2 TeamDelete 후 에러 방지
