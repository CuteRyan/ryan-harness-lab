...
5. **외부 리서치 의무 — 외부 사실 인용 시에만 적용** (#014 + 2026-05-05 turn 10 사용자 정정): 추천이 **외부 사실** (라이브러리 동작·모범 사례·최신 통계·공식 문서·CVE·표준 규격·문제 상황 회의 자료·아이디어 발굴 자료 等) 인용 시 = 자기 지식 (training data) 단언 금지 → **WebSearch / WebFetch / 외부 CLI** 1순위 → 보조 순서로 외부 자료 검색 후 출처 인용 의무. **내부 메타 작업 (HANDOFF·history·체크리스트·정합 grep·운영 정리·세션 인계 等) = 외부 리서치 면제 예외 (§아래 섹션 적용 의무)** = Read·Grep·Glob·git 으로 충분. **무차별 외부 리서치 강제 = 사용자 의도 왜곡 + 시간 낭비 = 금지**. 글로벌 `~/.claude/rules/research-mandatory.md` §1 (의무 조건) + §4 (면제 예외) 둘 다 정합 적용 의무.
